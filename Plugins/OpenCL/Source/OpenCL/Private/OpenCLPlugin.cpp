...
void OpenCLPlugin::EnumerateDevices(TArray<FOpenCLDeviceData>& OutDevices, bool bForceRefresh /*= false*/)
{
	...
	for (i = 0; i < NumPlatforms; i++)
	{
		...
		lDevices = (cl_device_id*)malloc(sizeof(cl_device_id) * NumDevices);
		clGetDeviceIDs(Platforms[i], CL_DEVICE_TYPE_ALL, NumDevices, lDevices, NULL);

		DeviceIdsMemoryList.Add(lDevices);
		...
		for (j = 0; j < NumDevices; j++)
		{
			...
			Device.DeviceId = FString::Printf(TEXT("%d"), ::PointerHash(lDevices[j]));
			Device.RawDeviceId = lDevices[j];

			clGetDeviceInfo(lDevices[j], CL_DEVICE_NAME, 0, NULL, &ValueSize);
			Value = (char*)malloc(ValueSize);
			clGetDeviceInfo(lDevices[j], CL_DEVICE_NAME, ValueSize, Value, NULL);
			Device.DeviceName = ANSI_TO_TCHAR(Value);
			free(Value);

			clGetDeviceInfo(lDevices[j], CL_DEVICE_VERSION, 0, NULL, &ValueSize);
			Value = (char*)malloc(ValueSize);
			clGetDeviceInfo(lDevices[j], CL_DEVICE_VERSION, ValueSize, Value, NULL);
			Device.HardwareVersion = ANSI_TO_TCHAR(Value);
			free(Value);

			clGetDeviceInfo(lDevices[j], CL_DRIVER_VERSION, 0, NULL, &ValueSize);
			Value = (char*)malloc(ValueSize);
			clGetDeviceInfo(lDevices[j], CL_DRIVER_VERSION, ValueSize, Value, NULL);
			Device.SoftwareVersion = ANSI_TO_TCHAR(Value);
			free(Value);

			clGetDeviceInfo(lDevices[j], CL_DEVICE_OPENCL_C_VERSION, 0, NULL, &ValueSize);
			Value = (char*)malloc(ValueSize);
			clGetDeviceInfo(lDevices[j], CL_DEVICE_OPENCL_C_VERSION, ValueSize, Value, NULL);
			Device.OpenCLVersion = ANSI_TO_TCHAR(Value);
			free(Value);

			clGetDeviceInfo(lDevices[j], CL_DEVICE_MAX_COMPUTE_UNITS, sizeof(MaxComputeUnits), &MaxComputeUnits, NULL);
...
void OpenCLPlugin::RunKernelOnDevices(const FString& KernelString, const FString& KernelName, const FString& Args,
