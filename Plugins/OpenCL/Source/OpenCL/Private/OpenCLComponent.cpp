...
bool UOpenCLComponent::HasValidHardware()
{
    ...
    OpenCLPlugin::Get().EnumerateDevices(Devices);
...
TArray<FOpenCLDeviceData> UOpenCLComponent::EnumerateDevices()
{
    ...
    if (OpenCLPlugin::Get().IsAvailable())
    {
        OpenCLPlugin::Get().EnumerateDevices(Devices);
    }
    return Devices;
...
bool UOpenCLComponent::ReadKernelFromFile(const FString& FilePath, FString& OutKernelSource,
                                          bool bIsContentRelative /*= true*/)
{
...
void UOpenCLComponent::RunOpenCLKernel(const FString& Kernel, const FString& KernelName /*= TEXT("main")*/,
                                       const FString& InputArgs /*= TEXT("")*/)
{
    if (OpenCLPlugin::Get().IsAvailable())
    {
        OpenCLPlugin::Get().RunKernelOnDevices(Kernel, KernelName, InputArgs,
                                               [this](const FString& Result, bool Success)
                                               {
...
void UOpenCLComponent::WatchKernelFolder(const FString& ProjectRelativeFolder)
{
    ...
    FDirectoryWatcherModule& DirectoryWatcherModule = FModuleManager::Get().LoadModuleChecked<FDirectoryWatcherModule>(
